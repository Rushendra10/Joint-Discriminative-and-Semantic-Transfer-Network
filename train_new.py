...
#from model_updated import AlexNet
import utils
...
for epoch in range(epoch, 10000):
    ...
    #print("s_score is " + str(s_score.size()))
    #print("t_score is " + str(t_score.size()))
    #print("s_pred is " + str(s_pred.size()))
    #print("t_pred is " + str(t_pred.size()))
    ...
    if da:
        ...
        #print("s_logit is " + str(s_logit.size()))
        #print("t_logit is " + str(t_logit.size()))

        G_loss, D_loss, semantic_loss = model.adloss(s_logit, t_logit, s_feature, t_feature, ys, t_pred)

        Discr_loss = model.DiscrimLoss_optimized(s_feature, ys, batch_size)

        #print("ys is " + str(ys.size()))
        #print("t_pred is " + str(t_pred.size()))
    ...
    if epoch % 20 == 0:
        ...
        if da:
            ...
            output('epoch: {}, C_loss: {}, semantic_loss: {}, discr_loss: {}'.format(epoch, C_loss.item(), semantic_loss.item(), Discr_loss))
            #output('correct: {}, C_loss: {}, G_loss:{}, D_loss:{}, Gregloss: {}, Dregloss: {}, semantic_loss: {}, F_loss: {}'.format(
                #s_correct.item(), C_loss.item(), G_loss.item(), D_loss.item(),Gregloss.item(), Dregloss.item(), semantic_loss.item(), F_loss.item()))
            #output('correct: {}, C_loss: {}, semantic_loss: {}'.format(s_correct.item(), C_loss.item(), semantic_loss.item()))
